...
pub fn spawn_wall_collision(
    ...
    // 壁がどこにあるかを考えます
    // すばやく簡単に検索できるように、それらを HashSet に GridCoords として保存します
    //
    // このマップのキーは、壁が属するレベルのエンティティになります。
    // これにより、結果として生じる衝突エンティティに 2 つの影響が生じます:
    // 1. 壁がレベルの境界に沿って分割されるようになります
    // 2. 衝突エンティティを適切なレベル エンティティの子として簡単に追加できるようになります
    ...
    wall_query.iter().for_each(|(&grid_coords, parent)| {
        ...
        // intgrid タイルの直接の親は、レベル エンティティではなく、レイヤー エンティティになります
        // レベル エンティティを取得するには、タイルの親が必要です。
        // ここで、parent_query が役立ちます。
